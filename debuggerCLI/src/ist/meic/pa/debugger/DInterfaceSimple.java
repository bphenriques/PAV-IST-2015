...
public final class DInterfaceSimple {
	...
	public Object run(Class<?> targetClass, Object target, Class<?> returnType,
		...
		DebuggerCLIStackManager.push(new MethodPrint(targetClass, methodName,
				args));
		...
		try {
			...
			while (debug) {
				try {
					...
					Command command = debugMethod(e.getCause(), targetClass,
							target);
	...
	private Command debugMethod(Throwable thrownException,
		...
		while (true) {
			...
			try {
				Command c = cm.executeCommand(thrownException, input,
						targetClass, target);
